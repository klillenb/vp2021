<?php
	...
	function upload_news($news_title, $news, $expire_date, $file_name){
		$response = null;
		$photo_id = null;
		...
		echo $conn->error;
		$stmt->bind_param("isssi", $_SESSION["user_id"], $news_title, $news, $expire_date, $photo_id);
		echo $stmt->error;
		echo $conn->error;
		if($stmt->execute()){
			$response = "Uudis salvestatud!";
		} else {
